...
export class BehaviorGroupPanel {
  get title() {
    const suffix = " - " + this.environment;
    if (this.artifact) {
      return (
        (this.artifact.getArtifact()?.getName() || "New behavior group") +
        " - Behavior Group" +
        suffix
      );
    } else {
      return "Behavior Group Designer" + suffix;
    }
  }
  ...
  private artifact: ttfCore.BehaviorGroup | null = null;
